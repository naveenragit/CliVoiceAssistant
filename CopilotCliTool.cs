...
public static class CopilotCliTool
{
    ...
    /// </summary>
    public static EmbeddedTerminal? Terminal { get; set; }
    ...
    /// </summary>
    public static string? ResumeSessionId
    {
        get => _sessionId;
    ...
    /// </summary>
    public static async Task<string> SendAsync(string message, bool submit = true)
    {
        ...
        if (Terminal is { IsRunning: true })
        {
            try
            {
                ...
                await Terminal.SendTextAsync(fullMessage, pressEnter: submit);
                ...
                return $"{{ \"success\": true, \"method\": \"terminal_keystroke\", \"message\": \"Command typed into Copilot CLI terminal. The user can see the response on screen.\" }}";
            }
            catch (Exception ex)
            {
        ...
        try
        {
            ...
            if (!string.IsNullOrWhiteSpace(response))
            {
                ...
                return $"{{ \"success\": true, \"chars\": {response.Length}, \"method\": \"prompt_mode\", \"response\": {JsonSerializer.Serialize(response)} }}";
            }
            else
            {
                AppLog.Warn("CopilotCliTool: prompt mode returned empty response");
                return "{ \"error\": \"empty response from prompt mode\" }";
            }
        }
        catch (Exception ex)
        {
            ...
            return $"{{ \"error\": {JsonSerializer.Serialize(err)} }}";
        }
    }
    ...
    /// </summary>
    private static async Task<string> RunPromptModeAsync(string copilotPath, string prompt)
    {
        ...
        {
            AppLog.Info($"CopilotCliTool: process exited with code {proc.ExitCode}");
